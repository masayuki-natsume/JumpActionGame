...
//背景を表示、引数で受け取ったｽｺｱｰを表示、ﾀｯﾁしたらGameScreenに遷移
class ResultScreen(private val mGame: JumpActionGame, private val mScore: Int) : ScreenAdapter() {
    ...
    init {
        if (mGame.mRequestHandler != null) { //RequestScreenが表示の時
            mGame.mRequestHandler.showAds(true) //ARHｲﾝﾀｰﾌｪｲｽの抽象ﾒｿｯﾄﾞを呼ぶAdMobを表示
        } // ←
    ...
    override fun render(delta: Float) {
        ...
        if (Gdx.input.justTouched()) {
            if (mGame.mRequestHandler != null) { // ←
                mGame.mRequestHandler.showAds(false) //AdMobを非表示
            }
