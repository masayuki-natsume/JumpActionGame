...
class Player(texture: Texture, srcX: Int, srcY: Int, srcWidth: Int, srcHeight: Int)
    ...
    companion object {
        ...
        // 状態（ジャンプ中、落ちている最中）
        val PLAYER_STATE_JUMP = 0        //ジャンプ中
        val PLAYER_STATE_FALL = 1        //落下中

        // 速度
        val PLAYER_JUMP_VELOCITY = 11.0f //ｼﾞｬﾝﾌﾟの速度定数
        val PLAYER_MOVE_VELOCITY = 20.0f //横方向の速度定数
    }

    private var mState: Int               //状態を保持の定義

    init {
        setSize(PLAYER_WIDTH, PLAYER_HEIGHT) //ｻｲｽﾞ指定
        mState = PLAYER_STATE_FALL           //状態指定
    }

    //表示する位置の決定と状態が変わるかの確認
    fun update(delta: Float, accelX: Float) { //描画を行うﾒｿｯﾄﾞ(render)から呼ばれる

        // 重力をプレイヤーの速度に加算し、速度から位置を計算する
        velocity.add(0f, GameScreen.GRAVITY * delta) //重力
        velocity.x = -accelX / 10 * PLAYER_MOVE_VELOCITY  //ﾌﾟﾚｲﾔｰ速度
        setPosition(x + velocity.x * delta, y + velocity.y * delta)//+で座標の設定(Spriteｸﾗｽのﾒｯｿﾄﾞ)
